---
title: Articles
layout: default
nav_order: 5
has_children: false
---

# Technical Articles & Publications

A collection of my technical writing on AI, security, and software development topics.

## Security & Cryptography

### [Wireless Network Security in 2025 and Beyond](https://medium.com/@dmontg/wireless-network-security-in-2025-and-beyond-71f7c13f9889?sk=c89972083bc34df9a7e81d4a30c0da66)
Exploring the evolving landscape of wireless security, emerging threats, and future-proof protection strategies.

### [Deep Dive: Fundamentals and the Future of Hashing and Cryptography](https://medium.com/@dmontg/deep-dive-fundamentals-and-the-future-of-hashing-and-cryptography-94ad3e458a7e?sk=cbfb0c889123e9f19c6382d343f02feb)
A comprehensive exploration of cryptographic hashing, from basic principles to quantum-resistant algorithms.

### [Quick Start Guide to WiFi Password Cracking: Techniques, Tools, and Advanced Attacks](https://medium.com/@dmontg/quick-start-guide-to-wifi-password-cracking-techniques-tools-and-advanced-attacks-9be021b55b46)
Educational guide on WiFi security testing methodologies and defense strategies.

## Model Context Protocol (MCP)

### [DeepSeek MCP Server: Circumventing "Server Busy" Errors and Keeping Your Data Private](https://medium.com/@dmontg/deepseek-mcp-server-circumventing-server-busy-errors-and-keeping-your-data-private-c52dc7981ba2)
Learn how to run DeepSeek models locally with MCP integration, avoiding rate limits and maintaining data privacy.

### [How To Install Model Context Protocol (MCP) in 10 minutes or less](https://medium.com/@dmontg/how-to-install-model-context-protocol-mcp-in-10-minutes-or-less-9a8c0560a371)
Step-by-step guide to getting started with MCP quickly and efficiently.

## AI & Large Language Models

### [Running An LLM Locally and Connecting to MCP: Without touching the Command Terminal](https://medium.com/@dmontg/running-an-llm-locally-and-connecting-to-mcp-without-touching-the-command-terminal-f3e076a8a4fc)
GUI-based approach to local LLM deployment for non-technical users.

### [Decoding Quantization: Understanding all Those Qs and Ks, and the Art of Running LLMs Locally](https://medium.com/@dmontg/decoding-quantization-understanding-all-those-qs-and-ks-and-the-art-of-running-llms-locally-c8391f099ed7)
Deep dive into quantization techniques and their impact on model performance and resource usage.

## Infrastructure & AI Governance

### [AI Co-ops: A Radical Approach to Community-Owned AI](https://medium.com/@dmontg/ai-co-ops-a-radical-approach-to-community-owned-ai-b4a2b07d27b8)
Exploring cooperative models for democratizing AI development and ensuring community control.

## Writing Philosophy

I focus on making complex technical topics accessible while maintaining technical accuracy. My articles often include:
- Practical, hands-on tutorials
- Real-world use cases
- Performance benchmarks
- Security considerations
- Future implications

## Follow My Writing

- [Medium Profile](https://medium.com/@dmontg) - Regular technical articles
- [GitHub](https://github.com/DMontgomery40) - Code samples and repositories
- [LinkedIn](https://linkedin.com/in/dmontgomery40) - Professional updates

---

[← Back to Home]({{ site.baseurl }}/) | [View Projects →]({{ site.baseurl }}/Projects/)
